      ...
      <el-form ref="from" :model="from" label-width="80px">
        ...
        <el-form-item label="宿管老师">
          <el-select v-model="from.dormitoryTeacherId" placeholder="请选择" style="width:350px;">
            <el-option v-for="(item, key) in list4" :key="key" :value="item.id"  :label="item.username">{{ item.username }}</el-option>
          </el-select>
        </el-form-item>
        <el-form-item>
...
export default {
  ...
  methods: {
    ...
    showNr() {
      var that = this;
      this.$axios({
        url: `http://122.112.253.28:8095/prod-api/basedata/bdormitory/list?pageNum=1&pageSize=10&buildingNo=${that.query.buildingNo}&storey=${that.query.storey}&dormitoryNo=${that.query.dormitoryNo}`,
        method: "GET",
        headers: {
          Authorization: window.sessionStorage.token
        },
        data: {
          buildingNo: that.query.buildingNo,
          storey: that.query.storey,
