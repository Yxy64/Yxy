    ...
    <div class="nav">
      ...
      <div class="xk">
        ...
        <Select style="width:200px" class="sele" v-model="building">
          <Option v-for="(item,key) in list" :key="key" :value="item.dictSort" :label="item.dictLabel">{{item.dictLabel}}</Option>
        </Select>
        <label class="lab">楼层</label>
        <Select style="width:200px" class="sele" v-model="storey">
          <Option v-for="(item,key) in list2" :key="key" :label="item.dictLabel" :value="item.dictSort">{{item.dictLabel}}</Option>
        </Select>
        <label class="lab">宿舍号</label>
        <Input placeholder="请输入宿舍号" style="width: 180px" v-model="bdormitory" />
        <br />
        <br />
    ...
    <div class="ss-xx" v-show="!showZk">
      <div class="sslc" v-for="(item, key) in list3" :key="key">
        ...
        <div class="zt-ss">
          ...
          <el-badge :value="item.leaveCount" class="item">
            <el-button size="small">请假</el-button>
          </el-badge>
          <el-badge :value="item.comebacklate" class="item">
...
export default {
  ...
  methods: {
    ...
    loadData3() {
      ...
      this.$axios({
        url:
          `http://122.112.253.28:8095/prod-api/smartdor/sdAttence/listAbsenceDormitory?building=${that.building}&storey=${that.storey}&bdormitory=${that.bdormitory}`,
